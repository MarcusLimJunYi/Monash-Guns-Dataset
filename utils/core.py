...
from data import COCODetection, VOCDetection, MGDDetection, detection_collate, preproc
...
def set_optimizer(net, cfg):
    if cfg.optimizer.type == 'SGD':
        return optim.SGD(net.parameters(),
                     lr = cfg.train_cfg.lr_sgd[0],
                     momentum = cfg.optimizer.momentum,
                     weight_decay = cfg.optimizer.weight_decay)

    elif cfg.optimizer.type == 'Adam':
        return optim.Adam(net.parameters(),
                            lr = cfg.train_cfg.lr_adam[0])
...
def adjust_learning_rate(optimizer, gamma, epoch, step_index, iteration, epoch_size, cfg):
    global lr
    if cfg.optimizer.type == 'SGD':
        lr_init = cfg.train_cfg.lr_sgd[0]
    elif cfg.optimizer.type == 'Adam':
        lr_init = cfg.train_cfg.lr_adam[0]

    if epoch <= 5:
        lr = cfg.train_cfg.end_lr + (lr_init-cfg.train_cfg.end_lr)\
         * iteration / (epoch_size * cfg.train_cfg.warmup)
    else:
        for i in range(len(cfg.train_cfg.step_lr.MGD)): #COCO
            if cfg.train_cfg.step_lr.MGD[i]>=epoch:
                if cfg.optimizer.type == 'SGD':
                    lr = cfg.train_cfg.lr_sgd[i]
                elif cfg.optimizer.type == 'Adam':
                    lr = cfg.train_cfg.lr_adam[i]
                break
...
def get_dataloader(cfg, dataset, setname='train_sets'):
    ...
    Dataloader_function = {'VOC': VOCDetection, 'COCO':COCODetection,'MGD':MGDDetection}
    _Dataloader_function = Dataloader_function[dataset]
    if setname == 'train_sets':
        if dataset == 'VOC':
            dataset = _Dataloader_function(cfg.VOCroot,getattr(cfg.dataset, dataset)[setname], _preproc)

        elif dataset == 'COCO':
            dataset = _Dataloader_function(cfg.COCOroot,getattr(cfg.dataset, dataset)[setname], _preproc)
        
        elif dataset == 'MGD':
            dataset = _Dataloader_function(cfg.MGDroot,getattr(cfg.dataset, dataset)[setname], _preproc)
    else:
        if dataset == 'VOC':
            dataset = _Dataloader_function(cfg.VOCroot,getattr(cfg.dataset, dataset)[setname], None)

        elif dataset == 'COCO':
            dataset = _Dataloader_function(cfg.COCOroot,getattr(cfg.dataset, dataset)[setname], None)
        
        elif dataset == 'MGD':
            dataset = _Dataloader_function(cfg.MGDroot,getattr(cfg.dataset, dataset)[setname], None)
