...
class M2Det(nn.Module):
    ...
    def construct_modules(self,):
        ...
        if 'vgg' in self.net_family:
            ...
            shallow_in, shallow_out = 256,128 #W/8 x H/8
            medium_in, medium_out = 512,256 #W/8 x H/8
            deep_in, deep_out = 1024,512 #W/8 x H/8
        ...
        self.reduce_shallow= BasicConv(shallow_in, shallow_out, kernel_size=3, stride=1, padding=1)
        self.reduce_medium= BasicConv(medium_in, medium_out, kernel_size=3, stride=1, padding=1)
        ...
        self.leach = nn.ModuleList([BasicConv(
                    deep_out+medium_out+shallow_out,
                    self.planes//2,
    ...
    def forward(self,x):
        ...
        if 'vgg' in self.net_family:
            for k in range(len(self.base)):
                x = self.base[k](x)
                if k in self.base_out: #Extract features from layer 22 and 34
                    base_feats.append(x)
        elif 'res' in self.net_family:
            base_feats = self.base(x, self.base_out)
            
        base_feature = torch.cat(
                (F.interpolate(self.reduce_shallow(base_feats[0]),scale_factor=0.5,mode='nearest'),self.reduce_medium(base_feats[1]),
                    F.interpolate(self.up_reduce(base_feats[2]),scale_factor=2,mode='nearest')),1
                )
    ...
    def init_model(self, base_model_path):
        ...
        self.reduce_shallow.apply(weights_init)
        self.reduce_medium.apply(weights_init)
