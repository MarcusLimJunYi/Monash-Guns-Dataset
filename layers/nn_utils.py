...
class TUM(nn.Module):
    def __init__(self, first_level=True, input_planes=128, is_smooth=True, side_channel=512, scales=6):
        ...
        self.side_channel = side_channel #512
        self.input_planes = input_planes #256
        self.planes = 2 * self.input_planes
        self.first_level = first_level
        self.scales = scales
        self.in1 = input_planes + side_channel if not first_level else input_planes #768 if first_level, else is 256

        self.layers = nn.Sequential()
        self.layers.add_module('{}'.format(len(self.layers)), BasicConv(self.in1, self.planes, 3, 2, 1))
        for i in range(self.scales-2): #scales = 6, end at i=4
            if not i == self.scales - 3: #when i =/= 3
        ...
        for i in range(self.scales-2): #end at i=4
        ...
        if self.is_smooth: #True for m2det512_vgg
            smooth = list()
            for i in range(self.scales-1): #end at i=5
                smooth.append(
                        BasicConv(self.planes, self.planes, 1, 1, 0)
                        )
            self.smooth = nn.Sequential(*smooth) #pointer to list() of 6 scales

    def _upsample_add(self, x, y, fuse_type='interp'):
        _,_,H,W = y.size()
        if fuse_type=='interp':
            return F.interpolate(x, size=(H,W), mode='nearest') + y #upsample input x then add y
        else:
    ...
    def forward(self, x, y):
        if not self.first_level:
            x = torch.cat([x,y],1)
        conved_feat = [x] #array of (128,1,1) scale layer
        for i in range(len(self.layers)): #end at i=5
            x = self.layers[i](x)
            conved_feat.append(x) #add 5 scales layer to array
        
        deconved_feat = [self.toplayer[0](conved_feat[-1])] #(128,1,1) layer
        for i in range(len(self.latlayer)): #end at i=5
