...
MGD_CLASSES = ( '__background__', # always index 0 
                'pistol'
                )
...
class AnnotationTransform(object):
    ...
    def __init__(self, class_to_ind=None, keep_difficult=True):
        self.class_to_ind = class_to_ind or dict(
            zip(MGD_CLASSES, range(len(MGD_CLASSES))))
...
class MGDDetection(data.Dataset):
    ...
    def __init__(self, root, image_sets, preproc=None, target_transform=AnnotationTransform(),
        ...
        for (year, name) in image_sets:
            self._year = year
            rootpath = os.path.join(self.root, 'MGD' + year) 
    ...
    def _write_voc_results_file(self, all_boxes):
        for cls_ind, cls in enumerate(MGD_CLASSES):
            cls_ind = cls_ind 
    ...
    def _do_python_eval(self, output_dir='output'):
        ...
        if output_dir is not None and not os.path.isdir(output_dir):
            os.mkdir(output_dir)
        for i, cls in enumerate(MGD_CLASSES):
        ...
        print('Mean AR = {:.4f}'.format(np.mean(recs)))
        print('~~~~~~~~')
        print('Results:')
        for ap in aps:
            print('{:.3f}'.format(ap))
