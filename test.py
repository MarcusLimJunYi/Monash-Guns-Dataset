...
def test_net(save_folder, net, detector, cuda, testset, transform, max_per_image=300, thresh=0.005):
    ...
    print_info('=> Total {} images to test.'.format(num_images),['green','bold'])
    ...
    print_info('Begin to evaluate',['green','bold'])
    ...
    print_info('===> Evaluating detections',['green','bold'])
...
if __name__ == '__main__':
    #Remove Annotations cache
    annotations_cache_path = os.path.join(cfg.VOCroot,'annotations_cache')
    if os.path.exists(annotations_cache_path):
        shutil.rmtree(annotations_cache_path)
        print_info('===> Removing VOC annotations cache',['green','bold'])
    else:
        print_info('===> No VOC annotations cache found',['green','bold'])

    annotations_cache_path = os.path.join(os.getenv("HOME"),'data/coco_cache',)
    if os.path.exists(annotations_cache_path):
        shutil.rmtree(annotations_cache_path)
        print_info('===> Removing COCO annotations cache',['green','bold'])
        os.mkdir(annotations_cache_path)
    else:
        print_info('===> No COCO annotations cache found',['green','bold'])

    net = build_net('test',
                    size = cfg.model.input_size,
                    config = cfg.model.m2det_config)
    init_net(net, cfg, args.trained_model)
    print_info('===> Finished constructing and loading model',['green','bold'])
    net.eval()
